...
In the front-end we are going to begin with the project structure. Create a `Components` folder, it'll contain all application's components and make a `Pages` folder which will have all the pages of our client application. so let's make a `Create.tsx`, `Delete.tsx`, `Update.tsx` and a `index.tsx` files. If I were going 
