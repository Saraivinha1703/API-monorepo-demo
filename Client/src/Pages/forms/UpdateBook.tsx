import { useEffect, useRef } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { CustomInput } from '../../Components/CustomInput';
import { useForm, Controller } from 'react-hook-form';
import clsx from 'clsx';
import { CreateBookModel } from '../../Models/CreateBook';
import axios from 'axios';
import { CustomButton } from '../../Components/CustomButton';
import { BooksAndAuthorsModel } from '../../Models/BooksAndAuthors';

type UpdateBookFormProps = React.DetailedHTMLProps<
  React.FormHTMLAttributes<HTMLFormElement>,
  HTMLFormElement
>;
type RegistrationFormFields = CreateBookModel;

export const UpdateBookForm = (props: UpdateBookFormProps) => {
  const { bookId } = useParams();
  const navigate = useNavigate();
  const bookDefaultValues = useRef<BooksAndAuthorsModel>();

  const {
    control,
    handleSubmit,
    setValue,
    formState: { errors },
  } = useForm<RegistrationFormFields>();

  const onSubmit = handleSubmit((data: CreateBookModel) => {
    const bookDto = {
      id: bookId,
      name: data.name,
      price: data.price,
      rating: data.rating,
      createdDate: data.createdDate,
    };

    axios({
      method: 'put',
      baseURL: '/api/updateBook',
      data: bookDto,
      params: { authorId: data.authorId },
    }).then(() => navigate('/Read'));
  });

  useEffect(() => {
    axios<BooksAndAuthorsModel>({
      method: 'get',
      baseURL: '/api/getBook',
      params: { bookId },
    })
      .then(res => (bookDefaultValues.current = res.data))
      .then(() => {
        setValue(
          'name',
          bookDefaultValues.current?.name ? bookDefaultValues.current.name : ''
        );
        setValue(
          'price',
          bookDefaultValues.current?.price ? bookDefaultValues.current.price : 0
        );
        setValue(
          'rating',
          bookDefaultValues.current?.rating
            ? bookDefaultValues.current.rating
            : 0
        );
        setValue(
          'createdDate',
          bookDefaultValues.current?.createdDate
            ? bookDefaultValues.current.createdDate
            : new Date(Date.now())
        );
        setValue(
          'authorId',
          bookDefaultValues.current?.author.id
            ? bookDefaultValues.current.author.id
            : 0
        );
      });
  }, [bookDefaultValues, bookId, setValue]);

  return (
    <form
      onSubmit={onSubmit}
      {...props}
      className={clsx(
        'bg-gradient-to-r from-zinc-100 to-white w-1/2 mt-5 flex flex-col items-center gap-1 py-3 rounded-xl shadow-xl duration-300 shadow-black/30 hover:p-5 hover:bg-gradient-to-b mb-16',
        props.className
      )}
    >
      <div className="w-full flex flex-col gap-5 items-center">
        <h1 className="font-bold text-xl text-pastelGreen-500">Update Book</h1>
        <Controller
          name="name"
          control={control}
          rules={{ required: 'The book must have a name.' }}
          render={({ field: { onChange } }) => (
            <CustomInput.Root>
              <CustomInput.Label
                text="Name"
                className={errors.name ? 'text-red-500' : undefined}
              />
              <CustomInput.Content
                placeholder="Book's Name"
                defaultValue={bookDefaultValues.current?.name}
                styleType={errors.name ? 'error' : 'normal'}
                onChange={onChange}
              />
              {errors.name && (
                <CustomInput.ErrorMessage>
                  {errors.name.message}
                </CustomInput.ErrorMessage>
              )}
            </CustomInput.Root>
          )}
        />
        <Controller
          name="price"
          control={control}
          rules={{ required: 'The book must have a price.' }}
          render={({ field: { onChange } }) => (
            <CustomInput.Root>
              <CustomInput.Label
                text="Price"
                className={errors.price ? 'text-red-500' : undefined}
              />
              <CustomInput.Content
                styleType={errors.price ? 'error' : 'normal'}
                defaultValue={bookDefaultValues.current?.price}
                placeholder="How much does it cost? (00.00)"
                type="number"
                onChange={onChange}
              />
              {errors.price && (
                <CustomInput.ErrorMessage>
                  {errors.price.message}
                </CustomInput.ErrorMessage>
              )}
            </CustomInput.Root>
          )}
        />
        <Controller
          name="rating"
          control={control}
          rules={{
            required: 'The book must have a rating.',
            max: { value: 5, message: 'The range is between 0 and 5.' },
          }}
          render={({ field: { onChange } }) => (
            <CustomInput.Root>
              <CustomInput.Label
                text="Rating - optional"
                className={errors.rating ? 'text-red-500' : undefined}
              />
              <CustomInput.Content
                styleType={errors.rating ? 'error' : 'normal'}
                defaultValue={bookDefaultValues.current?.rating}
                placeholder="How good is it? (0-5)"
                type="number"
                onChange={onChange}
              />
              {errors.rating && (
                <CustomInput.ErrorMessage>
                  {errors.rating.message}
                </CustomInput.ErrorMessage>
              )}
            </CustomInput.Root>
          )}
        />
        <Controller
          name="createdDate"
          control={control}
          rules={{ required: 'The book must have a date.' }}
          render={({ field: { onChange } }) => (
            <CustomInput.Root>
              <CustomInput.Label
                text="Created Date"
                className={errors.createdDate ? 'text-red-500' : undefined}
              />
              <CustomInput.Content
                defaultValue={
                  bookDefaultValues.current?.createdDate
                    .toString()
                    .split('T')[0]
                }
                type="date"
                onChange={onChange}
                styleType={errors.createdDate ? 'error' : 'normal'}
              />
              {errors.createdDate && (
                <CustomInput.ErrorMessage>
                  {errors.createdDate.message}
                </CustomInput.ErrorMessage>
              )}
            </CustomInput.Root>
          )}
        />
      </div>

      <CustomButton.Root styleType="pink" className="mt-3">
        <CustomButton.Text text="Update Book" />
      </CustomButton.Root>
    </form>
  );
};
